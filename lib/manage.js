...
module.exports = {
    ...
    error_logs : (req, res)=>{
        var hour = req.params.days;

        if(hour == 1 || hour == 24 || hour ==48 ){
            // SELECT CONVERT_TZ(timestamp, '+00:00', '+09:00') FROM errors;
            db.query(`SELECT CONVERT_TZ(timestamp,'+00:00', '+09:00') FROM errors WHERE timestamp > now() - INTERVAL ${hour} HOUR`,(error, list)=>{
                if(error){
                    res.status(500).json({
                        "status_code": 500,
                        "result" : "error occured",
            ...
            db.query(`SELECT * FROM errors WHERE timestamp > now() - INTERVAL ${hour} DAY`,(error, list)=>{
                if(error){
                    res.status(500).json({
                        "status_code": 500,
                        "result" : "error occured",
            ...
            db.query(`SELECT * FROM errors`,(error, list)=>{
                if(error){
                    res.status(500).json({
                        "status_code": 500,
                        "result" : "error occured",
    ...
    delete_error_logs : (req, res)=>{
        db.query('DELETE FROM errors WHERE timestamp < now() - INTERVAL 7 DAY',(error, result)=>{
            if(error){
                logError(405, 'DB 처리 중 오류 발생', 'warn', '관리자 서버')
                res.status(500).json({
                    "status_code" : 500,
